...
abstract class TweetSet {
	...
	def mostRetweeted: Tweet = {
		val tweet = mostRetweetedAcc(new Tweet("dummy", "", -1))
		if (tweet.retweets < 0) throw new NoSuchElementException
		tweet
	}
...
object GoogleVsApple {
	...
	lazy val googleTweets: TweetSet = allTweets.filter(tweet => google.exists(x => tweet.text.contains(x)))
	lazy val appleTweets: TweetSet = allTweets.filter(tweet => apple.exists(x => tweet.text.contains(x)))
